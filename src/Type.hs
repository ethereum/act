...
splitBehaviour store (Definition contract iface@(Interface _ decls) iffs (Creates assigns) extStorage ensures invs) = do
  ...
  ensures <- mapM (\expr -> checkBool (getPosn expr) env expr) ensures
