...
base store constructor =
  "Definition " <> baseName <> " :=\n" <> indent
    -- <> T.intercalate ("\n" <> indent) (map (parens . defaultValue) (Map.elems (headval store)))
...
reachable claims =
  "Inductive reachable : State -> Prop :=\n"
    <> "| base : reachable BASE\n"
    <> T.intercalate "" (map reachableStep claims)
    <> "\n.\n"
  where
...
defaultValue (StorageMapping xs t) = "fun "
  ...
  <> " => "
  <> abidef t
defaultValue (StorageValue t) = abidef t

abidef :: AbiType -> T.Text
abidef (AbiUIntType _) = "0"
abidef (AbiIntType _) = error "TODO: signed integer default"
abidef AbiAddressType = "0"
abidef AbiStringType = "EmptyString"
abidef a = error "TODO: missing default values"
