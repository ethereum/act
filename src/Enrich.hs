...
mkEthEnvBounds vars = catMaybes $ mkBound <$> nub vars
  where
    ...
    mkBound e = case lookup e defaultStore of
      Just (Integer) -> Just $ bound (toAbiType e) (iIntEnv e)
      _ -> Nothing
...
mkStorageBounds store refs
  ...
  where
    ...
    fromItem item@(DirectInt contract name) = bound (abiType $ slotType contract name) (iIntStore item)
    fromItem item@(MappedInt contract name _) = bound (abiType $ slotType contract name) (iIntStore item)
...
mkCallDataBounds =
    concatMap
      ( \(Decl typ name) -> case metaType typ of
          Integer -> [bound typ (iIntVar name)]
          _ -> []
      )
